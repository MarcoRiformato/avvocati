<template>
<AppLayout title="Gestisci casi studio">
<AdminContainer>
        ...
        <div class=" px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row sm:items-center">
                <div class="flex-auto mb-4 sm:mb-0 mt-8">
                    <h1 class="text-4xl">Gestisci casi studio</h1>
                </div>
                <div class="mt-8 sm:ml-16 sm:mt-0 sm:flex-none">
                    <button type="button" @click="$inertia.visit(route('admin.cases.create'))" class="btn btn-primary">Crea nuovo caso studio</button>
                </div>
            </div>
            <div class="mt-2">
                <table class="min-w-full divide-y divide-gray-300">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr :caseItem="caseItem" @click="$inertia.visit(route('cases.show', caseItem.id))" v-for="caseItem in cases" :key="caseItem.id">
                            ...
                            <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                                <button @click.stop="$inertia.visit(route('admin.cases.edit', caseItem.id))" class="btn btn-secondary">Modifica</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
